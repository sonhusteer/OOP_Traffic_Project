...
public abstract class Vehicle {
    ...
    private void updateTurnManeuver(double deltaTime) {
        ...
        // curve.
        double motionSpeed = MathUtils.clamp(speed, 0.0, maxCommittedTurnSpeed());

        // Slower when exiting the FIVE_WAY roundabout to look more natural
        if (activeTurn.getIntersection() != null && activeTurn.getIntersection().getType() == Intersection.Type.FIVE_WAY) {
            double t = activeTurn.getT();
            if (t > 0.5) {
                double exitFactor = (t - 0.5) / 0.5;
                double speedMultiplier = 1.0 - 0.35 * Math.sin(exitFactor * Math.PI); // max 35% reduction at apex
                motionSpeed *= speedMultiplier;
            }
        }
        ...
        if (activeTurn.getIntersection() != null && activeTurn.getIntersection().getType() == Intersection.Type.FIVE_WAY) {
            double cx = activeTurn.getIntersection().getCenter().getX();
            double cy = activeTurn.getIntersection().getCenter().getY();
            Vector2D p0 = activeTurn.getStartPoint();
            Vector2D p3 = activeTurn.getEndPoint();

            double theta0 = Math.atan2(p0.getY() - cy, p0.getX() - cx);
            double theta3 = Math.atan2(p3.getY() - cy, p3.getX() - cx);

            double phi = Math.atan2(40.0, 51.23);
            double roadDiff = theta3 - theta0 - 2.0 * phi;
            
            // Normalize roadDiff to [-PI, PI]
            while (roadDiff > Math.PI) roadDiff -= 2.0 * Math.PI;
            while (roadDiff <= -Math.PI) roadDiff += 2.0 * Math.PI;

            // Force counter-clockwise rotation for the road index transition
            if (roadDiff > 0) {
                roadDiff -= 2.0 * Math.PI;
            }

            double deltaTheta = roadDiff + 2.0 * phi;

            double t = activeTurn.getT();
            double theta = theta0 + t * deltaTheta;

            double r0 = Math.hypot(p0.getX() - cx, p0.getY() - cy);
            double r3 = Math.hypot(p3.getX() - cx, p3.getY() - cy);
            double r_base = r0 + t * (r3 - r0);
            double r_mid = 65.0; // exact center of the roundabout lane
            double r = r_base + Math.sin(t * Math.PI) * (r_mid - r_base);

            position.setX(cx + r * Math.cos(theta));
            position.setY(cy + r * Math.sin(theta));

            double desiredAngle = Math.toDegrees(Math.atan2(-Math.cos(theta), Math.sin(theta)));

            // Blend angles at entrance and exit for a natural transition
            if (t < 0.3) {
                double blendFactor = t / 0.3;
                double sourceProgress = activeTurn.getSourceLane().getProgressOf(p0);
                double sourceAngle = activeTurn.getSourceLane().getAngleAt(sourceProgress);
                double diff = normalizeAngle(desiredAngle - sourceAngle);
                desiredAngle = sourceAngle + diff * blendFactor;
            } else if (t > 0.7) {
                double blendFactor = (t - 0.7) / 0.3;
                double targetAngle = activeTurn.getTargetLane().getAngleAt(activeTurn.getTargetEntryProgress());
                double diff = normalizeAngle(targetAngle - desiredAngle);
                desiredAngle = desiredAngle + diff * blendFactor;
            }
