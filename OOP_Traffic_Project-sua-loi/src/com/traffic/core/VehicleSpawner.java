...
public class VehicleSpawner {
    ...
    private final Map<Lane, Integer> lastDecisionIndexByLane = new HashMap<>();
    ...
    public int spawn(String type, Lane lane, SpawnPosition position,
        ...
        for (int i = 0; i < count; i++) {
            ...
            Vehicle.TurnDecision turnDecision = (turnMode == SpawnTurnMode.RANDOM)
                    ? getNextCycledDecisionForLane(lane)
                    : normalizeSpawnDecisionForLane(lane, toTurnDecision(turnMode));
    ...
    private Vehicle.TurnDecision getNextCycledDecisionForLane(Lane lane) {
        if (lane == null) {
            return Vehicle.TurnDecision.STRAIGHT;
        }
        if (lane.isStraightOnly() || !lane.hasAnyTurnRule()) {
            return Vehicle.TurnDecision.STRAIGHT;
        }
        
        java.util.List<Vehicle.TurnDecision> allowed = new java.util.ArrayList<>();
        boolean canStraight = lane.hasAllowedTurnDecision(Vehicle.TurnDecision.STRAIGHT);
        boolean canLeft = lane.hasAllowedTurnDecision(Vehicle.TurnDecision.LEFT);
        boolean canRight = lane.hasAllowedTurnDecision(Vehicle.TurnDecision.RIGHT);
        
        if (canStraight) allowed.add(Vehicle.TurnDecision.STRAIGHT);
        if (canLeft) allowed.add(Vehicle.TurnDecision.LEFT);
        if (canRight) allowed.add(Vehicle.TurnDecision.RIGHT);
        
        if (allowed.isEmpty()) {
            allowed.add(Vehicle.TurnDecision.STRAIGHT);
        }
        
        int nextIndex = lastDecisionIndexByLane.getOrDefault(lane, -1) + 1;
        nextIndex = nextIndex % allowed.size();
        lastDecisionIndexByLane.put(lane, nextIndex);
        
        return allowed.get(nextIndex);
    }
}
