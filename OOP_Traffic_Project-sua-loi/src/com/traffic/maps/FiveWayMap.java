...
public class FiveWayMap implements MapConfig {
    ...
    public FiveWayMap() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            // Đặt điểm cuối làn đi vào tại bán kính 65 để vào đúng tâm làn vòng xuyến (R_entry^2 + 40^2 = 65^2 => R_entry ≈ 51.23)
            double inEndX = CX + 51.23 * vx + oxIn;
            double inEndY = CY + 51.23 * vy + oyIn;
            ...
            // Tạo làn đi ra
            // Đặt điểm đầu làn đi ra tại bán kính 65 để khớp với tâm vòng xuyến
            double outStartX = CX + 51.23 * vx + oxOut;
            double outStartY = CY + 51.23 * vy + oyOut;
        ...
        // Thiết lập luật rẽ cho vòng xuyến ngã 5 để các xe quay vòng và phân chia hướng đi hợp lý
        for (int i = 0; i < 5; i++) {
            inLanes[i].setTurnTarget(ngaNam, Vehicle.TurnDecision.RIGHT, outLanes[(i + 4) % 5]);
            inLanes[i].setTurnTarget(ngaNam, Vehicle.TurnDecision.STRAIGHT, outLanes[(i + 2) % 5]);
            inLanes[i].setTurnTarget(ngaNam, Vehicle.TurnDecision.LEFT, outLanes[(i + 1) % 5]);
        }
