...
public class IntersectionOccupancy {
    ...
    public boolean canEnterIntersection(Vehicle vehicle,
        ...
        for (Vehicle other : vehicles) {
            ...
            // Do not block a vehicle merely because another vehicle is approaching
            // the same intersection from a crossing lane. Traffic lights and explicit
            // turn rules decide who may enter. Blocking all approaching flows here
            // creates a 4-way deadlock when several vehicles reach the stop lines
            // at the same time. Only vehicles that have already committed to the
            // conflict area are handled above.
        }
        return true;
    }
    ...
    public boolean isVehicleInIntersection(Vehicle vehicle,
        ...
        if (vehicle.getCurrentIntersection() == intersection
            ...
            // A vehicle that already exited the conflict core should not reserve
            // the whole intersection until it is 128 px away. Active turns are
            // handled above; after that, physical distance to the core is enough.
